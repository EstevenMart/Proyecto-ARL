...
public class AccidenteControlador extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String idMecanismo=request.getParameter("textId");
        String idAgentAcci=request.getParameter("textId");
        String idSitio=request.getParameter("textId");
   ...
   AccidenteVO aciVO;
        aciVO = new AccidenteVO(idAccidente, idMecanismo, idAgentAcci, idSitio, tipoAccidente, fechaHora,
  ...
  AccidenteDAO aciDAO = new AccidenteDAO(aciVO);  
   ...
   switch(opcion){
            ...
            request.getRequestDispatcher("registrarAccidente.jsp").forward(request, response);
        break;
       ...
       if(aciDAO.actualizarRegistro()){
            ...
            break;
            case 3:
                aciVO = aciDAO.consultarAccidente(dia);
                if (aciVO != null) {
                    request.setAttribute("accidente", aciVO);
                    request.getRequestDispatcher("ActualizarAccidente.jsp").forward(request, response);
                } else {
                    request.setAttribute("mensajeError", "El Dia  NO exite");
                    request.getRequestDispatcher("ConsultarAccidente.jsp").forward(request, response);
                }
                break;
